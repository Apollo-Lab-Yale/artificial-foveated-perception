# AFP Train and eval functions used in main.py 
# ------------------------------------------------------------------------
...
def train_one_epoch_vm(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        ...
        if wandb_log_fn is not None:
            is_log_step = (data_iter_step == 0) or ((data_iter_step + 1) % print_freq == 0) or ((data_iter_step + 1) == len(data_loader))
            if is_log_step:
                step_stats = {
                    'loss': loss_value,
                ...
                for k, v in loss_dict_reduced_scaled.items():
                    step_stats[k] = float(v)
                wandb_log_fn(step_stats, data_iter_step)
