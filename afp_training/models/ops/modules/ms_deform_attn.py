...
class MSDeformAttn(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        if self.mode != 'decode_vis':
            xavier_uniform_(self.output_proj.weight.data)
            constant_(self.output_proj.bias.data, 0.)
        else:
            xavier_uniform_(self.output_proj_frame.weight.data)
            constant_(self.output_proj_frame.bias.data, 0.)            
    ...
    def forward(self, query, query_box, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        elif self.mode == 'decode_vm':
            return self.decode_vm_forward(query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask)
    ...
    def decode_vm_forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        if len(query.shape)==3: 
            ...
            #### sampling_offsets [bz, nq, nheads, nlevels, npoints, 2]
            attention_weights = self.attention_weights(query).view(N, Len_q, self.n_heads, self.n_levels * self.n_points)
            #### attention_weights [bz, nq, nheads, nlevels * npoints]
            ...
            for samp_i in range(nf): 
                # perform deformable attention per frame
            ...
            #query [bz, nf, nq, C]
            N, nf, Len_q, _ = query.shape  # decoder: [bz,nf,300,256]
            N, nf, Len_in, _ = input_flatten.shape
            assert (input_spatial_shapes[:, 0] * input_spatial_shapes[:, 1]).sum() == Len_in
            value = self.value_proj(input_flatten)
            if input_padding_mask is not None:
