...
class ConvertColor(object):
    ...
    def __call__(self, fgr, pha, bgr):
        if self.current == 'RGB' and self.transform == 'HSV':
            fgr = cv2.cvtColor(fgr, cv2.COLOR_RGB2HSV)
            bgr = cv2.cvtColor(bgr, cv2.COLOR_RGB2HSV)
        elif self.current == 'HSV' and self.transform == 'RGB':
            fgr = cv2.cvtColor(fgr, cv2.COLOR_HSV2RGB)
...
class RandomResize(object):
    ...
    def __call__(self, fgrs, phas, bgrs):
        ...
        # Ensure we never upscale beyond the configured max_size when provided
        capped_max_size = self.max_size if self.max_size is not None else None
        return resize(fgrs, phas, bgrs, size, capped_max_size)
