...
def collate_fn_vm(batch):
    batch = list(zip(*batch))
    batch[0] = nested_tensor_from_tensor_list_vm(batch[0], size_divisibility=32)
    return tuple(batch)
...
def nested_tensor_from_tensor_list_vm(tensor_list: List[Tensor], size_divisibility=1, split=True):
    if split:
        tensor_list = [tensor.split(3,dim=0) for tensor in tensor_list]
        tensor_list = [item for sublist in tensor_list for item in sublist]
...
def init_distributed_mode(args):
    ...
    # Explicitly bind barrier to the local CUDA device for NCCL to avoid
    # "best-guess GPU" warnings and potential hangs with ambiguous mappings.
    if torch.cuda.is_available():
        try:
            torch.distributed.barrier(device_ids=[args.gpu])
        except TypeError:
            # Older torch versions may not support device_ids in barrier().
