...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, query_box, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        elif self.mode == 'decode_afp':
            return self.decode_afp_forward(query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask)
    ...
    def decode_afp_forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
    ...
    def decode_forward(self, query, query_box, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        if len(query_box.shape) == 3:
            ...
            assert len(query_box.shape) == 4
            N, nf, Len_q, _ = query_box.shape
            N, nf, Len_in, _ = input_flatten.shape
            assert (input_spatial_shapes[:, 0] * input_spatial_shapes[:, 1]).sum() == Len_in
            value = self.value_proj(input_flatten)
            if input_padding_mask is not None:
